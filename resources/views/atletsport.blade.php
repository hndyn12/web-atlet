    ...
    <section id="contact" class="py-10 bg-gray-50">
        <div class="container mx-auto px-4">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-gray-800 mb-4">Daftar Atlet {{ $sport->name }}</h2>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                @if ($atlets->isEmpty())
                    <div class="flex justify-center items-center h-40">
                        <p class="text-gray-500 text-center">Belum ada atlet untuk cabang olahraga ini.</p>
                    </div>
                @else
                    <div class="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 px-4 sm:px-6 lg:px-8">
                        @foreach (is_iterable($atlets) ? $atlets : [] as $atlet)
                            <div
                                ...
                                <div class="relative w-full h-64">
                                    <img src="{{ asset('storage/' . $atlet->image) }}" alt="Foto {{ $atlet->name }}"
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <span
                                                    class="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded font-semibold"
                                                    title="Nomor Induk Atlet">NIA: {{ $atlet->atlet_id }}</span>
                                            </div>
                                            <p class="text-gray-600 text-sm">{{ $sport->name }}</p>
                                        </div>
                                    </div>
